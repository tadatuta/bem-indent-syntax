    ...
    const result = lines.reduce((parsed, line) => {
        ...
        const node = strContent ? [strContent[1]] : line.split(/\s+/).reduce((acc, word) => {
            ...
            if (word.startsWith(naming.elemDelim) || word.startsWith(naming.modDelim)) {
                const parentName = parent && parent.block ||
                    acc[acc.length - 1] && acc[acc.length - 1].block ||
                    PARENT_FALLBACK;
...
function node2bemjson(node) {
    ...
    Object.assign(bemjson, getMods(bemjson));
    delete bemjson.modName;
    delete bemjson.modVal;
    ...
    node.forEach(item => {
        if ((item.block && item.block !== bemjson.block) ||
            ...
            Object.assign(bemjson, getMods(item));
        }
    });
